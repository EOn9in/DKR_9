...
class Main(tk.Frame):
    ...
    def view_records(self):
        self.db.c.execute("SELECT * FROM characters")
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert("", "end", values=row) for row in self.db.c.fetchall()]

    def delete_records(self):
        for selection_item in self.tree.selection():
            self.db.c.execute("DELETE FROM characters WHERE id=%s", (self.tree.set(selection_item, "#1"),))
            self.db.conn.commit()
            self.view_records()

    def search_records(self, name):
        name = "%" + name + "%"
        self.db.c.execute("SELECT * FROM characters WHERE name LIKE %s", (name,))
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert("", "end", values=row) for row in self.db.c.fetchall()]
...
class Update(Child):
    ...
    def default_data(self):
        try:
            self.db.c.execute("SELECT * FROM characters WHERE id=%s",
...
class DB:
    def __init__(self):
        self.conn = psycopg2.connect(dbname="postgres", user="postgres", password="TOTnot0671M", host="localhost")
        self.c = self.conn.cursor(cursor_factory=psycopg2.extras.DictCursor)
        self.c.execute(
         '''CREATE TABLE IF NOT EXISTS characters
                (id serial primary key, name text, pol text, game text, storona text, status text);'''
        )
        self.conn.commit()

    def insert_data(self, name, pol, game, storona, status):
        self.c.execute(
            '''INSERT INTO characters(name, pol, game, storona, status) VALUES (%s, %s, %s, %s, %s)''',
